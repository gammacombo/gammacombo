...
int main() {
  ...
  RooRealVar norm_constant_obs("norm_constant_glob_obs", "global observable of normalization constant", 1e-8, 1e-20,
                               1e-6);  // this is the observed value, the global observable
  RooRealVar norm_constant("norm_constant", "norm_constant", 1e-8, 1e-20,
                           1e-6);  // the normalization constant is a nuisance parameter.
  ...
  RooRealVar branchingRatio("branchingRatio", "branchingRatio", 1e-7, -1e-6,
                            0.0001);  // this is the branching ratio, the parameter of interest
  ...
  //
  RooDataSet& data =
      *mass_model.generate(RooArgSet(mass), 5000);  // we use a reference here to avoid copying the object
