...
PDF_Abs::~PDF_Abs() {
  ...
  for (int i = 0; i < parameters->getSize(); i++) { delete parameters->at(i); }
  delete parameters;

  // clean objects in the 'theory' container
  for (int i = 0; i < theory->getSize(); i++) { delete theory->at(i); }
  delete theory;

  // clean objects in the 'observables' container
  for (int i = 0; i < observables->getSize(); i++) { delete observables->at(i); }
  delete observables;
...
TString PDF_Abs::uniquifyThisString(TString s, int uID) {
  ...
  if (s.Contains(uniqueID))
    s.ReplaceAll(uniqueID, newUniqueID);
  else
    s = s + newUniqueID;
  return s;
}
