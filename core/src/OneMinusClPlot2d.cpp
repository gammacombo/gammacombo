...
OneMinusClPlot2d::OneMinusClPlot2d(const OptParser* arg, TString name, TString title)
  ...
  for (int i = fillcolor[0].size(); i < arg->combid.size(); i++) { makeNewPlotStyle("", kBlue - 7); }
...
void OneMinusClPlot2d::makeNewPlotStyle(const TString htmlColor, const int ROOTColor) {
  ...
  int newColor;
  if (htmlColor.EqualTo("ROOT"))
    newColor = currentNumberOfStyles;
  else if (ROOTColor > 0)
    newColor = ROOTColor;
  else
...
///
void OneMinusClPlot2d::drawGroup() { OneMinusClPlotAbs::drawGroup(0.775); }
...
void OneMinusClPlot2d::drawLegend() {
  ...
  for (int i = 0; i < histos.size(); i++) {
    ...
    if (histos.size() == 1) {
      ...
      if (styleId < arg->filltransparency.size())
        g->SetFillColorAlpha(fillcolor[0][styleId], 1. - arg->filltransparency[styleId]);
      ...
      if (scanners[i]->getTitle() != "noleg") { m_legend->AddEntry(g, legTitle, options); }
    }
  }
  if (arg->plotlegbox) {
    TPaveText* legbox =
...
void OneMinusClPlot2d::Draw() {
  ...
  if (arg->isQuickhack(14)) { m_mainCanvas->GetPad(0)->SetLeftMargin(0.16); }
