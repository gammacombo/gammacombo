...
void OptParser::parseArguments(int argc, char* argv[]) {
  ...
  TCLAP::ValueArg<int> ndivArg(
      ...
      "a '-'.",
      false, -1, "int");
  TCLAP::ValueArg<int> coverageCorrectionIDArg(
      "", "covCorrect",
      "Correct for the coverage given a flat transformation of the pvalue distribution. You can also pass a coverage "
      "correction point (see --covCorrectPoint)\n"
      "0: no correction (default)\n"
      "1: linear correction\n"
      "2: linear + exponential correction\n"
      "3: linear + 1/x correction\n",
      false, 0, "int");
  TCLAP::ValueArg<int> coverageCorrectionPointArg("", "covCorrectPoint", "Point to use for coverage correction", false,
                                                  0, "int");
  TCLAP::ValueArg<int> ndivyArg(
      "", "ndivy",
      "Set the number of axis divisions (y axis in 2d plots): "
      "ndivy=N1 + 100*N2 + 10000*N3, "
      "N1 = number of 1st divisions (N2 = 2nd, N3 = 3rd). Default is 407. To enable bin optimization, pre-pend "
      "a '-'.",
      false, -1, "int");
  TCLAP::ValueArg<int> plotidArg("p", "plotid",
                                 "Make control plot with given ID (see --controlplots). "
                                 "Available IDs are 1-6. If not given, all control plots are made.",
                                 false, 0, "int");
  TCLAP::ValueArg<int> digitsArg("s", "digits",
  ...
  TCLAP::ValueArg<int> idArg("", "id",
                             ...
                             false, -1, "int");
  TCLAP::ValueArg<int> nsmoothArg("", "nsmooth", "number of smoothings to apply to final 1-CL plot. Default: 1", false,
                                  1, "int");
  ...
  TCLAP::ValueArg<int> npointsArg("", "npoints",
                                  ...
                                  "for both axes are set. See also --npoints2dx and --npoints2dy.",
                                  false, -1, "int");
  TCLAP::ValueArg<int> npoints2dxArg("", "npoints2dx", "Number of 2D scan points, x axis. Default: 50", false, -1,
                                     "int");
  TCLAP::ValueArg<int> npoints2dyArg("", "npoints2dy", "Number of 2D scan points, y axis. Default: 50", false, -1,
                                     "int");
  TCLAP::ValueArg<int> npointstoyArg("", "npointstoy", "Number of scan points used by the plugin method. Default: 100",
                                     false, 100, "int");
  ...
  // --------------- switch arguments
  TCLAP::SwitchArg batcheosArg("", "batcheos", "When submitting batch jobs (for plugin) write the output to eos",
                               false);
  ...
  TCLAP::SwitchArg interactiveArg("i", "interactive",
                                  "Enables interactive mode (requires X11 session). Exit with Ctrl+c.", false);
  TCLAP::SwitchArg intprobArg("", "intprob",
                              "Use the internal (=Prob) chi2min histogram"
                              " instead of the chi2min from the toy files to evaluate 1-CL of the plugin method.",
                              false);
  ...
  TCLAP::SwitchArg plotmagneticArg("", "magnetic",
                                   "In 2d plots, enable magnetic plot borders which will "
                                   "attract the 2sigma curves.",
                                   false);
  ...
  TCLAP::SwitchArg scanforceArg("f", "scanforce", "Use a stronger minimum finding method for the Plugin method.",
                                false);
  ...
  TCLAP::SwitchArg plotpullsArg("", "pulls",
                                "Make a pull plot illustrating the consistency "
                                "of the best solution with the observables.",
                                false);
  TCLAP::SwitchArg lightfilesArg("", "lightfiles",
                                 "Produce only light weight root files for the plugin toys."
                                 " They cannot be used for control plots but save disk space.",
                                 false);
  ...
  TCLAP::SwitchArg prArg(
      "", "pr",
      "Enforce the physical range on all parameters (needed to reproduce "
      "the standard Feldman-Cousins with boundary example). If set, no nuisance will be allowed outside the "
      "'phys' limit. However, toy generation of observables is not affected.",
      false);
  ...
  TCLAP::SwitchArg smooth2dArg(
      "", "smooth2d", "Smooth 2D p-value or cl histograms for nicer contour (particularly useful for 2D plugin)",
      false);
  ...
  TCLAP::MultiArg<int> linestyleArg(
      "", "linestyle",
      "Set line style of the 1D and 2D contours to be used for the combination. Default is a solid line for all.",
      false, "int");
  TCLAP::MultiArg<int> pevidArg("", "pevid",
                                "ID of combination used for the profile likelihood"
                                "that determines the parameter evolution for the Plugin toy generation. If not given, "
                                "the --combid will be used. Use -u to get a list of possible choices.",
                                false, "int");
  TCLAP::MultiArg<int> qhArg(
      "", "qh",
      "Quick hacks.\n"
      ...
      "5: Test toy generation in the 1d plugin method. At each scan point, 10 toys are printed out- but nothing is "
      "fit.\n"
      ...
      "16: In parameter evolution plots, add also the full evolution over the scan, in addition to just plotting the "
      "best evolution.\n"
      "17: In 2D plots with the PLUGIN and PROB methods, plot the PLUGIN first then the PROB.\n"
      "18: In 2D plots with PLUGIN and PROB methods, set legend titles as PLUGIN and PROB instead of (Plugin) and "
      "(Prob).\n"
  ...
  TCLAP::MultiArg<int> asimovArg(
      "", "asimov",
      "Run an Asimov toy, in which all observables are set to "
  ...
  TCLAP::MultiArg<std::string> asimovFileArg(
      ...
      "Example: --asimovfile parameters.dat",
      false, "string");
  TCLAP::MultiArg<int> plotsolutionsArg(
      "", "ps",
      "Include solutions in the plots.\n"
      ...
      false, "int");
  TCLAP::MultiArg<int> plotsolnArg("", "plotsoln",
                                   "The corresponding solution you would like to put on the plots.\n"
                                   "Sometimes when there are multiple equivalent solutions you prefer to draw one "
                                   "specifically - use this option for that.",
                                   false, "int");
  TCLAP::MultiArg<int> plot2dclArg("", "2dcl",
                                   "Plot '2d' confidence level contours in 2d plots.\n"
                                   "2D plots only:\n"
                                   " 0: don't plot 2dcl\n"
                                   " 1: do plot 2dcl\n"
  ...
  npoints2dy = npoints2dyArg.getValue() == -1 ? (npointsArg.getValue() == -1 ? 50 : npointsArg.getValue())
                                              : npoints2dyArg.getValue();
  ...
  if (plotsolutions.size() == 1 && combid.size() > 1) {
    for (int i = 1; i < combid.size(); i++) { plotsolutions.push_back(plotsolutions[0]); }
  }
  ...
  // fill the remaining ones up with 0=don't plot solution
    ...
    for (int i = plotsolutions.size(); i < combid.size(); i++) { plotsolutions.push_back(0); }
  }
  ...
  if (plotsoln.size() == 1 && combid.size() > 1) {
    for (int i = 1; i < combid.size(); i++) {
      ...
      if (isAction("plugin") && !plotpluginonly) { plotsoln.push_back(plotsoln[0]); }
    }
  }
  ...
  // fill the remaining ones up with 0=don't plot 2dcl
    ...
    for (int i = plot2dcl.size(); i < combid.size(); i++) { plot2dcl.push_back(0); }
  }
