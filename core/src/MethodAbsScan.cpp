...
bool MethodAbsScan::loadScanner(TString fName)
{
  ...
  if ( scanVar2!="" ) hChi2min2d = (TH2F*)obj;
  else hChi2min = (TH1F*)obj;
...
///
/// Print the nuisances at a certain scan point in a c++ format.
/// This can be used to save good start points to help the Prob
/// method in difficult areas. This needs to be called after a
/// scan (or multiple scans) were done.
///
void MethodAbsScan::printNuisances(vector<float>& points) const
{
  cout << "\nMethodAbsScan::printNuisances() : NUISANCES for " << title << endl;
  for ( int i=0; i<points.size(); i++ )
  {
    int iBin = hCL->FindBin(points[i]);
    RooSlimFitResult *r = curveResults[iBin-1];
    if ( !r ){
      cout << "MethodAbsScan::printNuisances() : ERROR : no fit result at this scan point!" << endl;
      return;
    }
    TDatime date;
    cout << "\n    Code to set these nuisances" << endl;
    cout << "  -------------------------------" << endl;
    cout << "    // nuisances from " << name;
    cout << " (" << date.AsString() << ")" << endl;
    cout << "    // at var " << scanVar1 << "=" << points[i] << endl;
    TIterator *it = w->set(parsName)->createIterator();
    while ( RooRealVar* p = (RooRealVar*)it->Next() )
    {
      RooRealVar* p2 = (RooRealVar*)r->floatParsFinal().find(p->GetName());
      if ( !p2 ) p2 = (RooRealVar*)r->constPars().find(p->GetName());
      if ( !p2 )
      {
        cout << "MethodAbsScan::printNuisances() : ERROR : parameter not found: " << p->GetName() << endl;
        continue;
      }
      float val = p2->getVal();
      float err = p2->getError();
			printNuisanceCode(p2->GetName(), val);
    }
    cout << endl;
    delete it;
  }
...
/// 2d version.
///
void MethodAbsScan::printNuisances(vector<float>& pointsx, vector<float>& pointsy) const
{
  cout << "\nMethodAbsScan::printNuisances() : NUISANCES for " << title << endl;

  if ( pointsx.size() != pointsy.size() )
  {
    cout << "\nMethodAbsScan::printNuisances() : Error : vectors of x,y coordinates have different size." << endl;
    return;
  }

  for ( int i=0; i<pointsx.size(); i++ )
  {
    cout << "MethodAbsScan::printNuisances() : at scan point "
    << scanVar1 << "=" << pointsx[i] << ", "
    << scanVar2 << "=" << pointsy[i] << endl;

    int xBin = hCL2d->GetXaxis()->FindBin(pointsx[i]);
    int yBin = hCL2d->GetYaxis()->FindBin(pointsy[i]);

    if ( xBin<1 || xBin>nPoints2dx || yBin<1 || yBin>nPoints2dy ){
      cout << "MethodAbsScan::printNuisances() : ERROR : specified point is out of scan range." << endl;
      continue;
    }

    RooSlimFitResult *r = curveResults2d[xBin-1][yBin-1];
    if ( !r ) cout << "MethodAbsScan::printNuisances() : ERROR : no fit result at this scan point!" << endl;
